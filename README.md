...
另外，AstrBot v4.18.x 读取 `_conf_schema.json` 使用的是 AstrBot 自定义 schema（并非标准 JSON Schema）。旧版 schema 会触发 `string indices must be integers, not 'str'`。

本仓库已将 `_conf_schema.json` 调整为 AstrBot 兼容格式（`type: string/int/bool/list/object/template_list`），可在面板正确生成默认配置并加载插件。
